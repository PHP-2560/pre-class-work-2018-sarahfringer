...
```
 * Yes, this histogram matches the exponential function \(1-e^{-x}\). This makes sense because n is a very large number, 1.1 million, and as stated by the central limit theorem as the sample size increases, its average value will be that of the standard normal N(0,1).  
...
```
  ...
  * The data looks very similar to the histogram made above when just plotting the vector of big.exp.draws.1. The only difference if only big.exp.draws.1.mat is a matrix, whereas big.exp.draws.1 is a vector. Not only do the two histograms look similar to each other, but they also both look simiilar to the histogram of the function \(1-e^{-x}\). 
